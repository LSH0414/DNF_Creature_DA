...
다만 S1 -> S2 -> 선계 패키지 종료 순으로 변화만큼 앞으로도 변화할지에 대해서는 부정적. 그 이유는 에를리히, 슈므에 대한 공급처가 이제는 아라드 패스 뿐이며, 해당 지표에는 무제한 상품으로 바꾸기 이전의 기간제 상품도 포함되있기 때문에 신규 유저 유입이 아니라면 증가하기 어려움.
  ...
  특이하게 나타난 데이터는 이전 종결 크리쳐였던 순백나비의 본캐 제외시 감소율임. 이러한 결과는 다 캐릭터 육성 유저중 본캐(명성치가 가장 높은 캘기터)에 이전 종결 크리쳐를 장착한 경우 다른 캐릭터에는 신규 종결 혹은 가성비 크리쳐를 장착하고 있는 비율이 많다는 것을 의미한다. 만약 가성비 크리쳐 장착 비율이 많은 것이라면 위에서 확인한 단일 캐릭터만 키우는 유저와 비슷한 성향을 가진 유저라 볼 수 있음, 따라서 만일 패스 상품 구매 상승을 위한 상품 분석시 고려해야할 그룹군임.
  </ul> 
